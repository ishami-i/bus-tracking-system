...
CREATE TABLE users (
    ...
    user_code TEXT GENERATED ALWAYS AS ('user-' || id) STORED
...
CREATE TABLE routes (
    ...
    route_code TEXT GENERATED ALWAYS AS ('route-' || id) STORED
    name                VARCHAR(100) NOT NULL,
...
CREATE TABLE buses (
    ...
    bus_code TEXT GENERATED ALWAYS AS ('bus-' || id) STORED
...
CREATE TABLE drivers (
    ...
    driver_code TEXT GENERATED ALWAYS AS ('driver-' || id) STORED
...
CREATE TABLE passengers (
    ...
    passenger_code TEXT GENERATED ALWAYS AS ('passenger-' || id) STORED
    user_id         INT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
...
CREATE TABLE stops (
    ...
    stop_code TEXT GENERATED ALWAYS AS ('stop-' || id) STORED
...
CREATE TABLE trips (
    ...
    trip_code TEXT GENERATED ALWAYS AS ('trip-' || id) STORED
    route_id        INT REFERENCES routes(route_id) ON DELETE SET NULL,
...
CREATE TABLE passenger_events (
    ...
    event_code TEXT GENERATED ALWAYS AS ('event-' || id) STORED
...
ALTER TABLE trips
    ADD CONSTRAINT fk_trips_event
    FOREIGN KEY (event_id) REFERENCES passenger_events(event_id)
    DEFERRABLE INITIALLY DEFERRED;

-- GPS Logs (real-time location tracking per bus/trip)
CREATE TABLE gps_logs (
    log_id      SERIAL PRIMARY KEY,
    log_code TEXT GENERATED ALWAYS AS ('gpslog-' || id) STORED
...
CREATE TABLE bus_requests (
    ...
    request_code TEXT GENERATED ALWAYS AS ('request-' || id) STORED
